...
const objLat = (obj) => {
    var out =`my name is ${obj.firstName} ${obj.lastName} I am ${obj.age} YO, and I love ${obj.hobby}.`
        return out
};
...
const cvFormatter = (arr) => {
    ...
    for(let i=0;i<=arr.length -1; i++){
        ...
        if(p.lastName == null){
         ...
         if(p.yearsOfExperience>1){

             out = {fullName:fullname,tech:tech}
             out2.push(out);
        }
     
    
        // out = {`${p.firstName} ${p.LastName}`,p.tech}
}
return out2
};
...
const applicationsStatics = (arr) => {
    ...
    for(let i=0;i<=arr.length -1; i++){
        ...
        switch(p.tech){
            case "Java" :
                result.java_Devs+=1;break
                case "Python":
                    result.python_Devs+=1;break
                case ".Net":
                    result.dotNet_Devs+=1;break
                case "JS":
                    result.javaScript_Devs+=1;break
        }
        fullname= `${p.firstName} ${p.lastName}`
        if (fullname == null || p.yearsOfExperience<=1) {
            result.rejectedApplicants+=1
            
        }

    }
    total= result.python_Devs + result.javaScript_Devs + result.dotNet_Devs+ result.java_Devs
...
//   --------------------------------------------------------------------------------------------------  
    test.SchoolName= data.SchoolName
    var out ;
    ...
    for(let i=0;i<=data.grades.length-1;i++){
        ...
        for (let k=0;k<=u.length-1 ;k++){
            ...
            for(var j=0;j<= y.length-1 ;j++){
             ...
             out = (count / 14)
            let removedDecimal = Math.trunc(out);
          m.avg = removedDecimal
          count=0
